...
int KeyFrame::GetNeighborInGrid(const float &x, const float &y) //for culling
{
    int posX = round((x-mnMinX)*mfGridElementWidthInv);
    int posY = round((y-mnMinY)*mfGridElementHeightInv);

    int radius = 1;

    int num_neighbor = 0;
    int nMinCellX = max(0,posX-radius);                        
    int nMaxCellX = min(mnGridCols-1,posX+radius);

    int nMinCellY = max(0,posY-radius);
    int nMaxCellY = min(mnGridRows-1,posY+radius);

    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            num_neighbor += mGrid[ix][iy].size();
        }
    }
...
int KeyFrame::CountOccupiedGrid()
{
    int OcCount = 0;
    int gridNum = 0;
    for (int gridX=0; gridX < mnGridCols; gridX+=3)
    {
        bool OcFlag = false;
        vector<size_t> keyIDs;
        for (int gridY=0; gridY < mnGridRows; gridY+=3)
        {
            keyIDs.insert(keyIDs.end(), mGrid[gridX][gridY].begin(), mGrid[gridX][gridY].end());
        }
        if (!keyIDs.empty())
        {
            for (auto id: keyIDs)
...
int KeyFrame::getGridNum()
{
    int gridNum = 0;
    for (int gridX=0; gridX < mnGridCols; gridX+=4)
    {
        for (int gridY=0; gridY < mnGridRows; gridY+=3)
        {
            gridNum++;
