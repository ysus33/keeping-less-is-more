...
MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(pRefKF), mnVisible(1), mnFound(1), mbBad(false), mbCulled(false),
...
MapPoint::MapPoint(const cv::Mat &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnFound(1), mbBad(false), mbCulled(false), mpReplaced(NULL), mpMap(pMap)
{
    Pos.copyTo(mWorldPos);
...
void MapPoint::SetCullFlag()
{
    ...
    mpMap->addCulledMapPoints(this);
    mpMap->EraseMapPoint(this);
}


// for culling
int MapPoint::GetNeighbors(KeyFrame* pKF)
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(!mNeighbors.count(pKF)) 
        LOG(INFO) << "NOT CALCULATED!!!";
    return mNeighbors[pKF];
}
void MapPoint::CalNeighbor(KeyFrame* pKF)
{
    float u = pKF->mvKeysUn[mObservations[pKF]].pt.x;
    float v = pKF->mvKeysUn[mObservations[pKF]].pt.y;

    const int neighbor =pKF->GetNeighborInGrid(u,v);
    mNeighbors[pKF]=neighbor;
...
MapPoint::MapPoint() : nObs(0), mnTrackReferenceForFrame(0),
                           mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0),
                           mnCorrectedReference(0), mnBAGlobalForKF(0), mnVisible(1), mnFound(1), mbBad(false),mbCulled(false),
